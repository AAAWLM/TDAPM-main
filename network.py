...
class domain_aggregation(nn.Module):
    def __init__(self,src_num):
        super(domain_aggregation, self).__init__()
        self.domain_weights = nn.Parameter(torch.randn(src_num))

    def forward(self, x):
        output = torch.matmul(x, torch.abs(self.domain_weights))
...
class DomainClassifier(nn.Module):
    def __init__(self, domain_num, bottleneck_dim=256, type="wn"):
        ...
        self.type = type
        if type == 'wn':
            self.fc = nn.utils.weight_norm(nn.Linear(bottleneck_dim, domain_num), name="weight")
            self.fc.apply(init_weights)
        else:
            self.fc = nn.Linear(bottleneck_dim, domain_num)
            self.fc.apply(init_weights)
    ...
    def forward(self, x):
        x = self.fc(x)
        # 添加 softmax 层，将输出转换为概率分布
        x = F.softmax(x, dim=1)
        return x

class domain_discriminator(nn.Module):
    def __init__(self, bottleneck_dim=256, type="linear"):
        super(domain_discriminator, self).__init__()
        self.type = type
        self.fc0 = nn.Linear(bottleneck_dim, 64)
        self.fc0.apply(init_weights)
        self.relu = nn.ReLU()
...
class domain_classifier_adv(nn.Module):
    def __init__(self, domain_num, bottleneck_dim=256, type="wn"):
        super(domain_classifier_adv, self).__init__()
        self.type = type
        if type == 'wn':
            self.fc = weightNorm(nn.Linear(bottleneck_dim, domain_num), name="weight")
            self.fc.apply(init_weights)
...
class domain_classifier_2(nn.Module):
    def __init__(self, domain_num, bottleneck_dim=256, type="wn"):
        ...
        self.type = type
        if type == 'wn':
            self.fc = weightNorm(nn.Linear(bottleneck_dim, 1), name="weight")
            self.fc.apply(init_weights)
        else:
